...
export default function RegraPainel({
  regra, anterior, proxima, destaque, termoBuscado, ehAdmin, userId, onEditar, onSelecionar,
}: {
  regra: Regra;
  anterior: Regra | null;
  proxima: Regra | null;
  ...
  // conta os trechos grifados sempre que a regra ou a busca mudam
  useEffect(() => {
    setAtual(-1);
  ...
  return (
    <div ref={raiz} className={`regra-painel fonte-${fonte}`}>
      ...
      {destaque.length > 0 && (
        <div className="regras-achados" role="status">
          {total > 0 ? (
            ...
            <span>as palavras buscadas estão só no título ou em outra regra</span>
          )}
        </div>
      )}

      <RegraTexto key={`texto-${regra.id}`} texto={regra.conteudo} destaque={destaque} recolhivel />
      ...
      <RegraComentarios key={`comentarios-${regra.id}`} regraId={regra.id} userId={userId} ehAdmin={ehAdmin} />
    </div>
  );
}

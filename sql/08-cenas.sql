-- DRUKALE / CENAS. Execute o arquivo INTEIRO no SQL Editor do Supabase.
-- Antes de rodar, substitua APENAS o e-mail abaixo pelo seu e-mail de administrador.
-- Não cria usuários nem apaga cenas, personagens ou locais.
begin;
...
begin
  select id into admin_id from auth.users where lower(email) = lower('COLE_SEU_EMAIL_DE_ADMIN_AQUI');
  if admin_id is null then
    raise exception 'Substitua COLE_SEU_EMAIL_DE_ADMIN_AQUI pelo e-mail de uma conta existente em Authentication > Users.';
  end if;
  insert into public.drukale_admins(user_id) values (admin_id) on conflict do nothing;
end $$;
...
-- As políticas restritivas são combinadas com as antigas por AND.
-- Contas de jogadores não podem usar as permissões antigas de administração.
do $$
declare tabela text;
begin
  foreach tabela in array array['characters','locais'] loop
    if to_regclass('public.' || tabela) is not null then
      execute format('alter table public.%I enable row level security', tabela);
      execute format('drop policy if exists drk_so_admin_insert on public.%I', tabela);
      execute format('create policy drk_so_admin_insert on public.%I as restrictive for insert to public with check (public.drk_e_admin())', tabela);
      execute format('drop policy if exists drk_so_admin_update on public.%I', tabela);
      execute format('create policy drk_so_admin_update on public.%I as restrictive for update to public using (public.drk_e_admin()) with check (public.drk_e_admin())', tabela);
      execute format('drop policy if exists drk_so_admin_delete on public.%I', tabela);
      execute format('create policy drk_so_admin_delete on public.%I as restrictive for delete to public using (public.drk_e_admin())', tabela);
    end if;
  end loop;
end $$;
...
begin
  ...
  for a in select jsonb_array_elements(valor) loop
    ...
    if length(a->>'nome') not between 1 and 240
      ...
      or (a->>'caminho') !~ ('^' || dono::text || '/' || cena_id::text || '/[0-9a-f-]+\.(jpg|png|webp|gif|mp4|webm)$') then return false; end if;
  end loop;
  return true;
end $$;
...
-- Envio apenas na pasta da conta conectada. Arquivos publicados ficam imutáveis.
...
-- Isola este bucket mesmo que existam políticas antigas mais abertas no Storage.
...
-- Busca no banco inteiro, com paginação. Não depende dos 20 cards visíveis.
create or replace function public.drk_normalizar_cena(v text) returns text
language sql immutable strict set search_path = '' as $$
select regexp_replace(translate(lower(btrim(v)), 'áàâãäéèêëíìîïóòôõöúùûüç', 'aaaaaeeeeiiiiooooouuuuc'), '\s+', ' ', 'g');
$$;
...
returns setof public.cenas language sql stable security invoker set search_path = '' as $$
  select c.* from public.cenas c
...
-- Verificação: deve mostrar seu UUID como administrador e o bucket cenas com 40 MB.
